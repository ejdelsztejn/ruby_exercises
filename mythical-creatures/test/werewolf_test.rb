...
class WerewolfTest < Minitest::Test
  ...
  def test_consumes_a_victim
    werewolf = Werewolf.new("David","London")
    victim = Victim.new("Warren")
    werewolf.change!
    assert_equal "#{victim.name} is yummy!", werewolf.consume(victim)
  end

  def test_cannot_consume_victim_if_in_human_form
    werewolf = Werewolf.new("David","London")
    victim = Victim.new("Warren")
    assert_equal "I can't eat #{victim.name}! How preposterous!", werewolf.consume(victim)
  end
